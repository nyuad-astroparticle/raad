...
void B4aDetectorConstruction::DefineMaterials()
{
  ...
  G4Element* Lu = new G4Element("Lutetium"  , "Lu", z=71 , a=170.93*g/mole);
...
G4Material* LYSO = new G4Material("LYSO", density=7.1*g/cm3,ncomponents=4);
  	LYSO->AddElement(Lu, number_of_atoms=1.8);
  	LYSO->AddElement(Y, number_of_atoms=0.2);
        LYSO->AddElement(Si, number_of_atoms=1);
        LYSO->AddElement(O, number_of_atoms=5);

G4Material* GAGG = new G4Material("GAGG", density=6.6*g/cm3,ncomponents=4);
  	GAGG->AddElement(Gd, number_of_atoms=3);
  	GAGG->AddElement(Al, number_of_atoms=2);
        GAGG->AddElement(Ga, number_of_atoms=3);
        GAGG->AddElement(O, number_of_atoms=12);
...
G4VPhysicalVolume* B4aDetectorConstruction::DefineVolumes()
{
  ...
  G4Material* LYSO = G4Material::GetMaterial("LYSO"); //ADRIANO
  G4Material* GAGG = G4Material::GetMaterial("GAGG"); //ADRIANO
